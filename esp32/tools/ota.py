...
def update_body(image, url):
    ...
    # Byte 297 names the board: 0 the universal image, or a CAT_INTSAT ID (1 NEO, 2 ZED/X20,
    # 3 MAX) for a test image. A device refuses another board's image; the universal image
    # fits every board.
    marker = data[288:304]
    if marker[:9] != b"NVFOTA1\0\x02" or marker[9] not in (0, 1, 2, 3) or marker[10:] != b"\x01\x00\x03\x00\x00\x00":
