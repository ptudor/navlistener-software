...
def report(**changes):
    value = {
        ...
        "board_uid_kind": "serial128", "board_uid": "00112233445566778899aabbccddeeff",
        "board_uid_address": 0x50, "eeprom_address": 0x50,
        "eeprom_uid_kind": "serial128", "eeprom_uid": "00112233445566778899aabbccddeeff", "board_rev": 1, "mcu_family": 1, "mcu_mac": "348518010203",
...
class ReportTests(unittest.TestCase):
    ...
    def test_kind_names_follow_the_discovery_library(self):
        ...
        self.assertEqual(commission_report.validate(value)["board_uid"], uid)
        for old in ("eui64", "microchip_eui64", "microchip_cs128"):
            with self.assertRaises(ValueError):
                commission_report.validate(dict(value, board_uid_kind=old, eeprom_uid_kind=old))

    def test_a_64_bit_identity_is_rejected(self):
        # Every board identity is a 128-bit factory serial.
        value = report(board_uid_kind="eui64", board_uid="0004a3aabbccddee",
                       eeprom_uid_kind="eui64", eeprom_uid="0004a3aabbccddee")
        with self.assertRaises(ValueError):
            commission_report.validate(value)
