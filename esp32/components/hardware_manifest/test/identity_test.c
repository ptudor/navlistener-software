...
int main(void) {
    ...
    assert(out.board_valid && out.eeprom_kind == NVF_UID_MICROCHIP_CS128 && out.board_address==0x50 && out.board_uid[2]==16);
    ...
    b=(bus){.model={4,0}};
    assert(!nvf_board_discover(read_id,&b,NULL,0,&out));
    assert(out.eeprom_kind==NVF_UID_ST_UID128 && out.board_uid[2]==16);
    memcpy(adopted,out.board_uid,sizeof adopted);
    assert(nvf_uid_observer(adopted,id));
    assert(!strcmp(id,"board-0004-20e00eff0405060708090a0b0c0d0e0f"));
    b.model[0]=0;b.model[1]=4;
    assert(!nvf_board_discover(read_id,&b,adopted,0,&out) && out.board_address==0x51);
    assert(nvf_board_discover(read_id,&b,adopted,NVF_UID_MICROCHIP_CS128,&out));
    ...
    puts("typed UID and read-only discovery: CS128, ST UID128, EUI64, faults and adoption PASS");
}
