                            ...
                            uint8_t board, const char *project)
{
    ...
    // The universal image fits every board; another only the board the device's manifest
    // names (board 0, no board named, matches only the universal image).
    bool fits = m[9] == NVF_BOARD_ID_UNIVERSAL || m[9] == board;
