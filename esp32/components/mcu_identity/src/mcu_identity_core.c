...
bool nvf_commission_encode(const nvf_commission_statement_t *s, uint8_t out[NVF_COMMISSION_STATEMENT_SIZE])
{
    ...
    put_be(out + 10, s->generation, 4); put_be(out + 14, s->commissioned_at, 8);
    memcpy(out + 22, s->atecc_serial, 9); memcpy(out + 31, s->rtc_eui64, 8);
    memcpy(out + 39, s->board_eui64, 8); memcpy(out + 47, s->mcu_mac, 6);
    memcpy(out + 53, s->mcu_key_sha256, 32); memcpy(out + 85, s->secure_boot_keys, 32);
    memcpy(out + 117, s->attestation, 32);
    return true;
}
...
bool nvf_commission_parse(const uint8_t *in, size_t len, nvf_commission_statement_t *out)
{
    ...
    nvf_commission_statement_t s = { .profile = in[1], .mcu_family = in[2], .mcu_key_alg = in[3],
        ...
        .security = (uint16_t)get_be(in + 8, 2), .generation = (uint32_t)get_be(in + 10, 4),
        .commissioned_at = get_be(in + 14, 8) };
    memcpy(s.atecc_serial, in + 22, 9); memcpy(s.rtc_eui64, in + 31, 8);
    memcpy(s.board_eui64, in + 39, 8); memcpy(s.mcu_mac, in + 47, 6);
    memcpy(s.mcu_key_sha256, in + 53, 32); memcpy(s.secure_boot_keys, in + 85, 32);
    memcpy(s.attestation, in + 117, 32);
...
{
    if (!s || !live) return "identity is missing";
