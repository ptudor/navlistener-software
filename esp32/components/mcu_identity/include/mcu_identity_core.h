...
//
// docs/COMMISSIONING.md is normative for every layout here: the 149-byte statement (§4), the
// 221-byte record, the session proof digest (§5.2) and the GNF1 EVIDENCE payload (§6). The Go
...
#define NVF_COMMISSION_STATEMENT_SIZE 149
...
typedef struct {
    ...
    uint8_t atecc_serial[9], rtc_eui64[8], board_eui64[8];
...
// The canonical observer id for an RTC EUI-64: lowercase hyphen-separated byte pairs.
...
typedef struct {
    bool atecc_valid, rtc_valid, board_valid, mac_valid, key_valid;
    uint8_t atecc_serial[9], rtc_eui64[8], board_eui64[8], mcu_mac[6], mcu_key_sha256[32];
} nvf_live_identity_t;
