...
/* Every board identity is a 128-bit factory serial. Kinds follow
 * esp_hardware_discovery's eeprom_factory_id_kind_t names (SERIAL128, ST_UID128);
 * these wire codes are fixed by the UID bytes. Code 1 is retired and never valid. */
enum { NVF_UID_SERIAL128 = 3, NVF_UID_ST_UID128 = 4 };
#define NVF_BOARD_UID_BYTES 16
...
static inline const char *nvf_uid_kind_name(uint16_t kind) {
    switch (kind) { case NVF_UID_SERIAL128: return "serial128"; case NVF_UID_ST_UID128: return "st_uid128"; default: return NULL; }
}
static inline bool nvf_uid_valid(const uint8_t uid[NVF_BOARD_UID_SIZE]) {
    if (!uid) return false;
    uint16_t kind = nvf_uid_kind(uid);
    size_t n = NVF_BOARD_UID_BYTES;
