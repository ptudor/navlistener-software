...
Retained files share the configured disk byte budget with the current file, and at most 256 replay files are admitted at startup (at that cap the current file is preserved but not archived, disk overflow is disabled for the run, and live delivery waits behind the admitted files). A fully acknowledged replay file that cannot be removed (permissions, an immutable flag) is retired from the replay list anyway after 30 seconds of failed removal, with one log line: its records are durably acknowledged, so the re-delivery after the next restart is dedup-safe, and the live session is never blocked behind it. Failures to read, validate, archive, or enumerate evidence disable new disk overflow for that run while fresh RAM capture continues. Original uncertain files remain for filesystem repair and a restart; they are never overwritten. Only a clean EOF permits discarding a torn final record; corrupt lengths, nonincreasing sequences, short session headers, and read errors preserve the file. Recognizable pre-session headerless records remain unsupported and are discarded with a diagnostic.

A crash between archive linking and unlinking can leave both names; recovery recognizes the same inode, and collector deduplication makes repeat delivery harmless. Append rollback can lose a torn tail but cannot make a new observation reuse an old identity. Orderly shutdown flushes only the current session's unacked RAM records and fsyncs them; existing replay files retain their original bytes. When that flush cannot spool (disk appends disabled after an incomplete recovery, or the live budget consumed by retained replay files) the dropped count is logged before exit rather than silently lost behind the "flushing" line. As before, unclean power loss may lose a page-cache/RAM tail, and configured overflow limits may drop records. Neither case extends a recovered session with new content.

Durable receipt tracking is capped at 1,024 sessions, 64 sessions per observer, 4,096 unresolved received sequences per session, and 65,536 unresolved sequences overall. A frame that would exceed these budgets closes the push stream before decoder handoff and is counted under `navlistener_durable_receipts_rejected_total{reason}`; it is not added to the watermark. Reconnects can always re-admit an already tracked hole, allowing historian recovery to release space. Fully resolved sessions idle for seven days are reclaimed. A session with unresolved holes is never acknowledged, but once it has been silent for one hour — far past every feeder's ACK-stall/reconnect window, so its replay copy is gone (a power-cut feeder restarts under a new session) — its holes are abandoned and counted under `navlistener_durable_holes_abandoned_total{source}`; a feeder that reappears with that session simply re-tracks what it replays. Without this, unrecoverable holes consumed the budget permanently and one crash-looping observer could fill the session table for every other feeder. Capacity sweeps are rate-limited to once per minute. The indexed per-session minimum makes ACK lookup constant time, while receipt and commit bookkeeping take logarithmic time within the fixed per-session budget. Live-only collectors retain receipt-based ACK behavior.

Audience projection stores suppress receiver-level decode, validation, CRC, capture-only, ionosphere-pair, and expiry counters, and do not write process-wide live-SV gauges. The operator store processes each physical input once and owns those metrics. Public event, public feed, organization, and collection stores still decode independently for disclosure isolation; no tenant identifiers are added to metric labels.
