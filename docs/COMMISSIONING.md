...
([hardware contract §4.1b](HARDWARE-OBSERVER.md)) proves that a board and its
secure element are original. It binds the ATECC serial, the RTC EUI-64, the
board EUI-64 and the board revision. It does not mention the microcontroller,
...
replaced with a blank one keeps all three attested identifiers, and its secure
...
| Part | Identity | Role |
|---|---|---|
| 24AA025E64 EUI-64 | board | the PCB. The permanent identity of a unit; a replaced PCB is a new unit. |
| MCP79412 EUI-64 | observer | the network name: certificate SAN and observer id. |
...
The microcontroller is a replaceable part with its own identity. Replacing it is
a recorded service event that produces a new commissioning record with the next
...
A fixed 149-byte big-endian layout. A layout change takes a new version and a
...
| 10 | 4 | generation | 1 at first commissioning; +1 each time the board is commissioned again |
| 14 | 8 | commissioned_at | Unix seconds, UTC |
| 22 | 9 | atecc_serial | |
| 31 | 8 | rtc_eui64 | |
| 39 | 8 | board_eui64 | |
| 47 | 6 | mcu_mac | factory base MAC; a label, never a proof |
| 53 | 32 | mcu_key_sha256 | SHA-256 of the key's DER SubjectPublicKeyInfo; zero when `mcu_key_alg` is 0 |
| 85 | 32 | secure_boot_keys | SHA-256 over the three 32-byte Secure Boot key digests in slot order, an empty or revoked slot as 32 zero bytes; zero when Secure Boot is off |
| 117 | 32 | attestation | SHA-256 of the board's 72-byte slot-14 record |
...
```text
SHA-256( "MFG-COMMISSION-v1" || statement[149] )
```

with ECDSA P-256. The ASCII prefix is domain separation: the same key also signs
slot-14 attestations (`"ATECC-MFG-ATTEST-v2"`), and neither signature can be
presented as the other.

The **record** is what is stored, carried and published — 221 bytes:

```text
statement[149] || signer_key_id[8] || R[32] || S[32]
```
...
**fingerprint** is the SHA-256 of all 221 bytes.
...
| 4 | the record's RTC EUI-64, rendered as an observer id, equals the authenticated observer | `identity` |
...
```json
{
  ...
  "boards": [{
    ...
    "profile": "trusted",
    "generation": 1,
    "record": "<base64 of the 221-byte record>"
  }]
}
```
...
- The descriptive columns must agree with the embedded record, and a board or an
  observer may appear only once. A registry that breaks either rule is rejected
  whole.
