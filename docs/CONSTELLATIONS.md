...
receiver), so the collector skips step 1 above and goes straight to parity/CRC + parameter
extraction. Block IDs (`sbf.go` dispatch; the block ID's low 13 bits, ignoring the 3-bit
revision):
