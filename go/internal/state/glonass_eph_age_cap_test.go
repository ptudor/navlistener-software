...
func TestGLONASSFrozenTbGuard(t *testing.T) {
	...
	// un-fired eph_aged). Reception is still live, so the wall switch does not
	// apply — the clamp must hold the served age at the +720 ceiling.
	...
	if sv.EphAgeM == nil || *sv.EphAgeM != 720 {
		t.Errorf("eph_age_m = %v at +13 h frozen tb, want the 720 min clamp (negative alias would un-fire eph_aged)", sv.EphAgeM)
	}
}
