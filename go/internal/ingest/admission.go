...
type observerPolicy struct {
	...
	credentials map[policyCredential]struct{}
}
...
func (p *PushServer) admit(ctx context.Context, current identity.ObserverContext, lookedUpAt uint64, cancel context.CancelFunc, credentials ...policyCredential) *Admission {
	p.authorizationMu.Lock()
	...
	for _, credential := range credentials {
		...
		if policy.credentials == nil {
			policy.credentials = make(map[policyCredential]struct{})
		}
		policy.credentials[credential] = struct{}{}
	}
