...
func TestCommissionVerify(t *testing.T) {
	...
	for name, c := range f.Cases {
		out, err := capture(t, "commission-verify", "-key", manufacturer, "-record", c.Record)
		if err != nil {
	...
	if _, err := capture(t, "commission-verify", "-key", filepath.Join(dir, "registry.pem"), "-record", trusted); err == nil {
		t.Error("record verified under a key that did not sign it")
	}
	if _, err := capture(t, "commission-verify", "-key", filepath.Join(dir, "registry.pem"), "-key", manufacturer, "-record", trusted); err != nil {
	...
	if _, err := capture(t, "commission-verify", "-key", manufacturer, "-record", hex.EncodeToString(tampered)); err == nil || !strings.Contains(err.Error(), "signature") {
		t.Errorf("tampered record: err = %v, want a signature failure", err)
	}
	for _, args := range [][]string{
		{"commission-verify", "-record", trusted},
		{"commission-verify", "-key", manufacturer},
		{"commission-verify", "-key", manufacturer, "-record", trusted[:40]},
	} {
...
func TestRegistryVerify(t *testing.T) {
	...
	out, err := capture(t, "registry-verify", "-key", filepath.Join(dir, "registry.pem"), "-file", registry)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := capture(t, "registry-verify", "-key", filepath.Join(dir, "manufacturer.pem"), "-file", registry); err == nil {
	...
	if _, err := capture(t, "registry-verify", "-key", filepath.Join(dir, "registry.pem"), "-file", altered); err == nil {
		t.Error("altered registry verified")
	}
	if _, err := capture(t, "registry-verify", "-key", filepath.Join(dir, "registry.pem"), "-file", filepath.Join(dir, "absent.json")); err == nil {
